...
uint8_t stepsize = 5;   // stepsize is step(pixel) per one meter (pixel/meter) 5
                        // is 0.2 meter per pixel
uint8_t celllength = 1; // cell length (meter) of cellmap
uint8_t cellsize =
    stepsize * celllength; // pixel number of cell, +1 is for axis path
cv::Mat pathmap(cellsize + 2, cellsize + 2, CV_16S, cv::Scalar(32767));
int start_pose = 0, start_odom = 0, stop_sonar = 0, stop_bumper = 0,
    stop_laser = 0, start_laser = 0, start_bug1 = 0, slow_laser = 0,
    cell_status = 0, system_status = 4, odom_switch, switch_publish,
    reset_odom = 0, scan_match = 0, erode_param, curve_param, turn_status = 1,
    start_wall_follow = 1;
/////////////////////////wall_follow cross cell
/// parameters///////////////////////////////////////////////////////////
int robot_mode, sweep_mode = 0, next_corner = 1, last_corner,
                as_wall_sweep_status = 0;
float offset_bound = 0; // if go_straight to go_straight, zoom out boundary
float minedge_x = (-(float)celllength), minedge_y = (-(float)celllength / 2),
      maxedge_x = 0, maxedge_y = ((float)celllength / 2), crosscell_x,
      crosscell_y, edgepoint_x, edgepoint_y, robot_body_x = 0.20,
      robot_body_y = 0.20; // boundary parameters
int first_bump = -1;
...
float delta_x = 0, delta_y = 0, frame_x = maxedge_x,
...
int cell_coordinate(cv::Mat opencv_map, uint8_t metersize,
                    const uint16_t laser_shift_x, const uint16_t laser_shift_y);
int sweep_scan(ros::Publisher pub, int frequency, const float path_coordinate_x,
               const float path_coordinate_y, float kp_ang, float ki_ang,
               float kd_ang);
void cell_sweepscan(const ros::Publisher pub, const int rate,
                    cell_path_info *sweeppath, float kp_ang, float ki_ang,
                    float kd_ang);
void test_sweepscan(const ros::Publisher pub, const int rate, float kp_ang,
                    float ki_ang, float kd_ang);
void where_to_go(int x, int y, cv::Mat path, cell_path_info *pathpoint);
void bumper_behavior();
void laser_avoidance();
void wall_follow(float minedge_x, float minedge_y, float maxedge_x,
                 float maxedge_y, ros::Publisher pub, int frequency,
                 float kp_ang, float ki_ang, float kd_ang);
void findedge(float minedge_xx, float minedge_yy, float maxedge_xx,
              float maxedge_yy);
float dis_p2p(float x1, float y1, float x2, float y2);
typedef actionlib::SimpleActionClient<move_base_msgs::MoveBaseAction>
    MoveBaseClient;
void save_cellmap();
void increment_sweepscan(const ros::Publisher pub, const int rate, float kp_ang,
                         float ki_ang, float kd_ang, cell_path_info *sweeppath);
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_mode =
        n_slam.subscribe<test1::behavior>("/behavior", 50, modeCallback);

    ros::Subscriber bumper_pub =
        n_slam.subscribe<irobotcreate2::Bumper>("bumper", 50, bumperCallback);

    mode_pub = n_slam.advertise<test1::behavior>("/behavior", 100);
    pubMessage = n_slam.advertise<geometry_msgs::Twist>("/cmd_vel", 1000);
    pubMessageSwitch =
        n_slam.advertise<irobotcreate2::OdomSwitch>("/switch_odom", 1000);
    ...
    while (n_slam.ok()) {
        ...
        if (robot_mode == 0) {
            if (start_wall_follow != 0 & start_pose == 1 & start_laser == 1) {
                wall_follow(minedge_x, minedge_y, maxedge_x, maxedge_y,
                            pubMessage, ros_loop_rate, P_vel, I_vel, D_vel);
            }
            ...
            if (start_wall_follow == 0 & !start_sweep_scan & scan_match) {
                ...
                for (int i = 0; i < sumCrossPoint; i++) {
                    printf("cross[%d] = x: %f, y: %f, minx: %f, miny: %f, "
                           "Maxx: %f, Maxy: %f, discover: %d\n",
                           i, crosspoint[i].x, crosspoint[i].y,
                           crosspoint[i].MinBoundary_x,
                           crosspoint[i].MinBoundary_y,
                           crosspoint[i].MaxBoundary_x,
                           crosspoint[i].MaxBoundary_y, crosspoint[i].discover);
                    printf("num_cell: %d\n", CellNumber);
                }
            }
            if (start_sweep_scan & reset_odom != 5) {
                ...
                if (start_laser)
                    laser_avoidance();
                /*if (ros::Time::now().toSec()-sweep_scan_secs >
                1.0f/(float)ros_loop_rate & start_pose & cellpath.status != 2 &
                cell_status){
                      //int sweep_status =
                sweep_scan(pubMessage,ros_loop_rate,2.0,0,P_vel,I_vel,D_vel);
                      //printf("sweep_scan  secs =
                %f\n",ros::Time::now().toSec()-sweep_scan_secs);
                      //test_sweepscan(pubMessage,ros_loop_rate,P_vel,I_vel,D_vel);
                      cell_sweepscan(pubMessage, ros_loop_rate,
                &cellpath,P_vel,I_vel,D_vel);
            ...
            if (cellpath.status == 2 & cell_status == 0) {
                int crosscell_status =
                    sweep_scan(pubMessage, ros_loop_rate, crosscell_x,
                               crosscell_y, P_vel, I_vel, D_vel);
                if (crosscell_status >= 1) {
                    ...
                    /*for (int i = 0 ; i < pathmap.rows ; i++){
                            for (int j = 0 ; j < pathmap.cols ; j++){
                                    ...
                                    pathmap.at<int16_t>(cv::Point2i(i,j)) =
                    32767;
                            }
                            //printf("\n");
                    }*/
            ...
            if (cellpath.status == 2 & behavior.robotmode != 2 & 0) {
                if (status1 == 0)
                    status1 = sweep_scan(pubMessage, ros_loop_rate, 0.20, 0.5,
                                         P_vel, I_vel, D_vel);
                if (status1 == 1) {
                    status2 = sweep_scan(pubMessage, ros_loop_rate, 0.15, 0.5,
                                         P_vel, I_vel, D_vel);
                    if (status2 == 1) {
                        printf("dock ready!!");
...
void bumper_behavior() {
    if (stop_bumper >= 1) {
        ...
        if (odom_switch == 0)
            printf("stop_bumper = %d,bumper warning in odom(x = %f , y = %f), "
                   "go to next setpoint\n",
                   stop_bumper, odom.pose.pose.position.x,
                   odom.pose.pose.position.y);
        else
            printf("stop_bumper = %d,bumper warning in pose(x = %f , y = %f), "
                   "go to next setpoint\n",
...
void poseCallback(const geometry_msgs::PoseStamped &msg) {
    ...
    if ((reset_odom != 0)) {
        ...
        if (reset_odom != 5) {
            if ((pose_yaw<M_PI / 4 & pose_yaw>(-1) * M_PI / 4) |
                pose_yaw<(-3) * M_PI / 4 | pose_yaw> 3 * M_PI / 4)
                reset_odom = 3;
            else
                reset_odom = 2;
        }
        if (fabs(pose.pose.position.x - odom.pose.pose.position.x) < 0.08 &
                fabs(pose.pose.position.y - odom.pose.pose.position.y) < 0.08 |
            reset_odom == 1 | reset_odom == 3 | reset_odom == 5) {
            ...
            printf("Switch_odom.status=%d,yaw=%f,pose.x=%f,pose.y=%f,odom.x=%f,"
                   "odom.y=%f,x_diff=%f,y_diff=%f\n",
                   Switch_odom.status, pose_yaw, pose.pose.position.x,
                   pose.pose.position.y, odom.pose.pose.position.x,
                   odom.pose.pose.position.y,
                   fabs(pose.pose.position.x - odom.pose.pose.position.x),
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan) {
    laser = *scan;
    /*static int count_stop_laser = 0;  /////////////////////// Lidar obstacle
    avoidance ////////////////////////////
    for (int i = 0 ; i<35 ; i+=1)
    {
    printf("stop_laser = %d,count_stop_laser = %d,laser.ranges[%d] =
...
void imageCallback(const nav_msgs::OccupancyGrid &msg) {
    mapmsg = msg;
    // save_cellmap();
    // printf("scan matching ... , position.x = %f, position.y =
...
void laser_avoidance() {
    float frontdatar[5] = {0}, frontdatal[5] = {0}, sidedatar1[5] = {0},
          sidedatar2[5] = {0}, sidedatal1[5] = {0}, sidedatal2[5] = {0};
    float frontdata1[5] = {0}, frontdata2[5] = {0}, frontdata3[5] = {0},
          frontdata4[5] = {0}, frontdata5[5] = {0};
    float frontdata6[5] = {0}, frontdata7[5] = {0}, frontdata8[5] = {0},
          frontdata9[5] = {0}, frontdata10[5] = {0};
    for (int j = 0; j < 5; j += 1) {
        // Right side
    ...
    float laser_[6] = {sidedistl2, sidedistl1, frontdistl,
                       frontdistr, sidedistr1, sidedistr2};
    ...
    bool stop_laser_status = (frontdistr >= 0.1 & frontdistr <= 0.32) |
                             (frontdistl >= 0.1 & frontdistl <= 0.32) |
                             (frontdist5 >= 0.1 & frontdist5 <= 0.32) |
                             (frontdist10 >= 0.1 & frontdist10 <= 0.32) |
                             (frontdist3 >= 0.1 & frontdist3 <= 0.32) |
                             (frontdist8 >= 0.1 & frontdist8 <= 0.32) |
                             (frontdist1 >= 0.1 & frontdist1 <= 0.32) |
                             (frontdist6 >= 0.1 & frontdist6 <= 0.32);
    if (stop_laser_status) {
        slow_laser = 1;
    } else if ((frontdistr >= 0.1 & frontdistr <= 0.37) |
               (frontdistl >= 0.1 & frontdistl <= 0.37) |
               (frontdist4 >= 0.1 & frontdist4 <= 0.4) |
               (frontdist9 >= 0.1 & frontdist9 <= 0.4))
        slow_laser = 2;
    else if ((frontdistr >= 0.1 & frontdistr <= 0.42) |
             (frontdistl >= 0.1 & frontdistl <= 0.42) |
             (frontdist3 >= 0.1 & frontdist3 <= 0.47) |
             (frontdist8 >= 0.1 & frontdist8 <= 0.47))
        slow_laser = 3;
    else if ((frontdistr >= 0.1 & frontdistr <= 0.47) |
             (frontdistl >= 0.1 & frontdistl <= 0.47) |
             (frontdist2 >= 0.1 & frontdist2 <= 0.51) |
             (frontdist7 >= 0.1 & frontdist7 <= 0.51))
        slow_laser = 4;
    else if ((frontdistr >= 0.1 & frontdistr <= 0.52) |
             (frontdistl >= 0.1 & frontdistl <= 0.52) |
             (frontdist1 >= 0.1 & frontdist1 <= 0.57) |
             (frontdist6 >= 0.1 & frontdist6 <= 0.57))
        slow_laser = 5;
    else
        slow_laser = 0;
    if (start_bug1) {
        ...
        if (turn_status == 1) {
            printf("laser_ = %f, %f, %f, %f, %f, %f\n", laser_[0], laser_[1],
                   laser_[2], laser_[3], laser_[4], laser_[5]);
            for (int i = 0; i < 6; i++) {
    ...
    // printf("fr = %f, fl = %f, fd1 = %f, fd3 = %f,fd5 = %f, fd6 = %f,fd8 = %f,
    // fd10 = %f, slow_laser =
...
void setpointTF(float &input_x, float &input_y) {
    ...
    float x_TF = 0.0f + frame_x - offset,
          y_TF = (float)celllength / 2 + frame_y - offset;
...
void cell_sweepscan(const ros::Publisher pub, const int rate,
                    cell_path_info *sweeppath, float kp_ang, float ki_ang,
                    float kd_ang) {
    float x_sp = (float)(sweeppath->x - 1) / stepsize,
          y_sp = (float)(sweeppath->y - 1) / stepsize - delta_y;
    int status = 1;
    ...
    if (sweeppath->tan != sweeppath->next_tan & status_set) {
        ...
        // printf("Sweep scan in  x = %d, y = %d, x_sp = %f, y_sp =
    ...
    // printf("cell_sweepscan x_pre = %d, y_pre = %d, x = %d, y = %d, x_next =
    // %d, y_next = %d, tan = %f, next_tan =
...
void test_sweepscan(const ros::Publisher pub, const int rate, float kp_ang,
    ...
    if (start_pose == 1) {
        ...
        int status =
            sweep_scan(pub, rate, path_coordinate_x[path_counter],
                       path_coordinate_y[path_counter], kp_ang, ki_ang, kd_ang);
...
void increment_sweepscan(const ros::Publisher pub, const int rate, float kp_ang,
                         float ki_ang, float kd_ang,
                         cell_path_info *sweeppath) {
    if (start_pose == 1) {
        static float cellstep = 0.2f, cellmax = 2.0f, cellmin = 0.0;
        static float x_sp = odom.pose.pose.position.x,
                     y_sp = odom.pose.pose.position.y;
        sweeppath->x = (int16_t)(pose.pose.position.x * stepsize + 1 +
                                 0.5); // 0.5 is for round
        sweeppath->y = (int16_t)(pose.pose.position.y * stepsize + 1 + 0.5);
        ////////////////////// sweep scan in the square //////////////////////
        // cellmax is the side length of square, cellstep is the step in one
        // sweep scan, x_sp, y_sp are the set points in x, y
        static uint8_t sweep_count = 0, xsp_count = 0;
        int status = sweep_scan(pub, rate, x_sp, y_sp, kp_ang, ki_ang, kd_ang);
        // printf("sweeppath.status = %d,x_sp = %f, y_sp = %f, sweeppath.x =
        // %d,sweeppath.y = %d, sweeppath.pre_x = %d, pathpoint.pre_y =
        // %d\n",sweeppath->status,x_sp,y_sp,sweeppath->x,sweeppath->y,sweeppath->pre_x,sweeppath->pre_y);
        /*if ((sweeppath->x != sweeppath->pre_x | sweeppath->y !=
        sweeppath->pre_y) & cell_status == 0 & status >= 1){
                sweeppath->pre_x = sweeppath->x;
                sweeppath->pre_y = sweeppath->y;
                where_to_go(sweeppath->x,sweeppath->y,pathmap,sweeppath);
                if (sweeppath->status == 1){
                        printf("movetogoal, x_sp = %f, y_sp =
        %f\n",(float)(sweeppath->x-1-0.5)/stepsize,(float)(sweeppath->y-1-0.5)/stepsize);
                        printf("min min min pose.pose.position.x = %f,
        pose.pose.position.y = %f\n",pose.pose.position.x,pose.pose.position.y);
                        x_sp = (float)(sweeppath->x-1-0.5)/stepsize;
                        y_sp = (float)(sweeppath->y-1-0.5)/stepsize;
                        usleep(5000000);
                        sweep_count = 0;
                        xsp_count = 0;
                        printf("finish finish finish x_sp = %f, y_sp = %f,
        pose.pose.position.x = %f, pose.pose.position.y =
            ...
            if (sweep_count % 2 == 0) {
                if (xsp_count % 2 == 0) {
                    if (status == 2) {
                        if (odom_switch == 0) {
                            ...
                            y_sp = odom.pose.pose.position.y;
                            printf("x+, odom.pose.pose.position.x = %f, "
                                   "odom.pose.pose.position.y = %f\n",
                            ...
                            printf("x+, pose.pose.position.x = %f, "
                                   "pose.pose.position.y = %f\n",
                                   pose.pose.position.x, pose.pose.position.y);
                        }
                    }
                ...
                if (status == 2) {
                    if (odom_switch == 0) {
                        ...
                        x_sp = odom.pose.pose.position.x;
                        y_sp = odom.pose.pose.position.y;
                        printf("y+, odom.pose.pose.position.x = %f, "
                               "odom.pose.pose.position.y = %f\n",
                               odom.pose.pose.position.x,
                               odom.pose.pose.position.y);
                    } else {
                        ...
                        x_sp = pose.pose.position.x;
                        y_sp = pose.pose.position.y;
                        printf("y+, pose.pose.position.x = %f, "
                               "pose.pose.position.y = %f\n",
                               pose.pose.position.x, pose.pose.position.y);
                    }
                }
...
void where_to_go(int x, int y, cv::Mat path, cell_path_info *pathpoint) {
    ...
    if (start >= 32766) {
        ...
        if (min == 32766 &
            pathmap.at<int16_t>(cv::Point2i(pathpoint->x, pathpoint->y)) ==
                32766) {
            pathpoint->status = 2;
            cell_status = 0;
    ...
    ///////////////////////////////// next point
    /////////////////////////////////////////
    pathpoint->tan = atan2((pathpoint->y - pathpoint->pre_y),
                           (pathpoint->x - pathpoint->pre_x));

    if (path.at<int16_t>(cv::Point2d(pathpoint->x, pathpoint->y - 1)) <
        next_start) {
        pathpoint->next_x = pathpoint->x;
        pathpoint->next_y = pathpoint->y - 1;
        next_start =
            path.at<int16_t>(cv::Point2d(pathpoint->x, pathpoint->y - 1));
    }
    if (path.at<int16_t>(cv::Point2d(pathpoint->x, pathpoint->y + 1)) <
        next_start) {
        pathpoint->next_x = pathpoint->x;
    ...
    if (path.at<int16_t>(cv::Point2d(pathpoint->x + 1, pathpoint->y)) <
        next_start) {
        pathpoint->next_x = pathpoint->x + 1;
        pathpoint->next_y = pathpoint->y;
        next_start =
            path.at<int16_t>(cv::Point2d(pathpoint->x + 1, pathpoint->y));
    }
    if (path.at<int16_t>(cv::Point2d(pathpoint->x - 1, pathpoint->y)) <
        next_start) {
        pathpoint->next_x = pathpoint->x - 1;
    ...
    pathpoint->next_tan = atan2((pathpoint->next_y - pathpoint->y),
                                (pathpoint->next_x - pathpoint->x));
    printf("where_to_go,x_pre =%d,y_pre = %d,x = %d,y = %d,x_next = %d,y_next "
           "= %d, tan = %f, next_tan = %f\n",
           pathpoint->pre_x, pathpoint->pre_y, pathpoint->x, pathpoint->y,
           pathpoint->next_x, pathpoint->next_y, pathpoint->tan,
           pathpoint->next_tan);
}
void maptransform(cv::Mat *opencv_map) {
    // rotation matrix
    ...
    double angle = 180;
    cv::Point2f center((uint16_t)((0 - mapmsg.info.origin.position.x) /
                                  mapmsg.info.resolution),
                       (uint16_t)((0 - mapmsg.info.origin.position.y) /
                                  mapmsg.info.resolution));
    ...
    t_mat.at<float>(0, 2) =
        (float)((float)celllength / 2 + frame_y) * (1 / mapmsg.info.resolution);
    t_mat.at<float>(1, 1) = 1;
    t_mat.at<float>(1, 2) =
        (float)(0.0f + frame_x) * (1 / mapmsg.info.resolution);
    printf("celllength = %d, frame_x = %f, frame_y = %f\n", celllength, frame_x,
           frame_y);
    cv::warpAffine(*opencv_map, *opencv_map, t_mat, dst_sz);
}
int cell_coordinate(cv::Mat opencv_map, uint8_t metersize,
                    const uint16_t laser_shift_x,
                    const uint16_t laser_shift_y) {
    ...
    if (opencv_map_x >=
        opencv_map.rows - blocksize / 2) // limit x by the image size
        opencv_map_x = opencv_map.rows - 1 - blocksize / 2;
    else if (opencv_map_x < blocksize / 2)
        opencv_map_x = blocksize / 2;
    if (opencv_map_y >=
        opencv_map.cols - blocksize / 2) // limit y by the image size
    ...
    cv::Mat imageROI =
        opencv_map(cv::Rect(laser_shift_y, laser_shift_x,
                            celllength * metersize, celllength * metersize));
    for (int16_t i = 0; i < imageROI.rows;
         i++) // resize original map to match cellmap
    {
        for (int16_t j = 0; j < imageROI.cols; j++) {
            if (imageROI.at<uint8_t>(cv::Point2i(i, j)) == 0) {
                cellmap_resize.at<int16_t>(
                    cv::Point2i(j / cell_map_ratio, i / cell_map_ratio)) = 0;
            } else if (imageROI.at<uint8_t>(cv::Point2i(i, j)) == 1)
                cellmap_resize.at<int16_t>(
                    cv::Point2i(j / cell_map_ratio, i / cell_map_ratio)) = 1;
            else if (imageROI.at<uint8_t>(cv::Point2i(i, j)) == 120)
                cellmap_resize.at<int16_t>(
                    cv::Point2i(j / cell_map_ratio, i / cell_map_ratio)) = 2;
        }
    }
    /*printf("imageROI
    ................................................................\n");
    ...
    for (int16_t j = 0; j < cellsize; j++) {
        for (int16_t i = 0; i < cellsize; i++) {
            if (j % 2 == 0) {
                if (cellmap_resize.at<int16_t>(cv::Point2i(i, j)) <= 2 |
                    pathmap.at<int16_t>(cv::Point2d(i + 1, j + 1)) == 32766)
                    cellmap.at<int16_t>(cv::Point2i(i, j)) = 32766;
                else
                    cellmap.at<int16_t>(cv::Point2i(i, j)) =
                        j * cellsize + i + 1;
            } else {
                if (cellmap_resize.at<int16_t>(cv::Point2i(i, j)) <= 2 |
                    pathmap.at<int16_t>(cv::Point2d(i + 1, j + 1)) == 32766)
                    ...
                    cellmap.at<int16_t>(cv::Point2i(i, j)) =
                        j * cellsize + i + 1; //(j+1)*cellsize-i;
            }
        }
    }
    /*printf("cellmap in cell
    coordinate..................................................................\n");
    ...
    // printf("cellmin = %f,cellmax = %f,minPoint = (%d,%d),maxPoint =
    // (%d,%d)\n",cellmin,cellmax,minPoint.x,minPoint.y,maxPoint.x,maxPoint.y);
    /*printf("pathmap in cell
    coordinate...........................................................\n");
...
int sweep_scan(ros::Publisher pub, int frequency, const float path_coordinate_x,
    ...
    static float prior_lin = 0, prior_ang_pos = 0, prior_ang_neg = 0,
                 ang_diff_pos = 0, ang_diff_neg = 0, ang_diff_max = 0, max_diff;
    ...
    // Calculates relative and total differences in distance as well as their
    // absolute values
    ...
    if (!(diff < diff_distance)) {
        ...
        if (diff<max_diff / 2 & reset_odom_param & scan_match & max_diff> 0.2 &
            !start_bug1) {
            reset_odom = 1;
            reset_odom_param = 2;
            scan_match = 0;
        }
        // printf("waypoint = (%f, %f), pose = (%f,
        ...
        // Calculates desired angle, difference from current angle and its
        // absolute value
        ...
        // printf("ang_desired_old = %f,ang_desired = %f,ang_desi_diff = %f,yaw
        // = %f,ang_diff = %f,step_one =
        // %d\n",ang_desired_old,ang_desired,ang_desired_old-fabs(ang_desired),yaw,ang_diff,step_one);
        if ((ang_desired_old - fabs(ang_desired) > 0.03 |
             ang_desired_old - fabs(ang_desired) < -0.03) &
            step_one == 0 & start_bug1) {
            printf("step_one = %d, start bug 1\n", step_one);
            step_one = 1;
        }
        if (step_one == 1 & ang_desired_old - fabs(ang_desired) <= 0.03 &
            ang_desired_old - fabs(ang_desired) >= -0.03 & start_bug1) {
            start_bug1 = 0;
            turn_status = 1;
            step_one = 0;
            rotate_mode = 1;
        ...
        // Checks which direction Roomba should rotate depending on whichever is
        // closer
        ...
        // Linear PID
        // Kicks in when the robot if at most 45 degrees off the correct
        // direction
        ...
        // if((abs_ang_diff < M_PI/4 && ((abs_diff > 0.1) || (abs_diff < 0.1)))
        // || (abs_ang_diff > M_PI*7/4 && ((abs_diff > 0.1) || (abs_diff <
        // 0.1))))
        if (rotate_mode == 0) {
            if (slow_laser == 1) {
                if (diff >= 0.6) {
                    start_bug1 = 1;
                    if (odom_switch == 0)
                        printf("start bug1 with obstacle in (x = %f , y = %f), "
                               "(x_sp = %f, y_sp = %f).......\n",
                               odom.pose.pose.position.x,
                               odom.pose.pose.position.y, path_coordinate_x,
                               path_coordinate_y);
                    else
                        printf("start bug1 with obstacle in (x = %f , y = %f), "
                               "(x_sp = %f, y_sp = %f).......\n",
                    ...
                    reset_odom_param = 0;
                    if (odom_switch == 0)
                        printf("stop by laser and arrival (x = %f , y = %f), "
                               "(x_sp = %f, y_sp = %f).......\n",
                               odom.pose.pose.position.x,
                               odom.pose.pose.position.y, path_coordinate_x,
                               path_coordinate_y);
            ...
            x_speed =
                kp_lin * abs_diff + integral_lin + kd_lin * derivative_lin;
    ...
    // Publishes message, increases ROS counter, then sleeps until next
    // iteration
...
void occupancyGridToGrayCvMat(const nav_msgs::OccupancyGrid *map, cv::Mat *im) {
    ...
    for (size_t i = 0; i < map->data.size(); i++) {
        if (data[i] == 0)
            im->at<uint8_t>(
                cv::Point2i(i / map->info.height, i % map->info.width)) =
                240; // Vec3d sequence == BGR, Scanned area
        else if (data[i] == 100)
            im->at<uint8_t>(cv::Point2i(
                i / map->info.height, i % map->info.width)) = 1; // Scanned edge
        else
            im->at<uint8_t>(
                cv::Point2i(i / map->info.height, i % map->info.width)) =
                120; // not Scanned area

        // just check if there is actually something in the map
        if (i != 0)
            mapHasPoints = mapHasPoints || (data[i] != testpoint);
        testpoint = data[i];
    }

    // sanity check
    if (!mapHasPoints) {
        ROS_WARN("map is empty, ignoring update.");
    }
}
...
void wall_follow(float minedge_xx, float minedge_yy, float maxedge_xx,
                 float maxedge_yy, ros::Publisher pub, int frequency,
                 float kp_ang, float ki_ang, float kd_ang) {
    static float start_x, start_y;
    static int status = 0;
    static int avoidance_status = 1;
    static double wall_follow_secs = ros::Time::now().toSec(),
                  wf_reset_odom_sec = ros::Time::now().toSec();
    if (dis_p2p(pose.pose.position.x, pose.pose.position.y,
                odom.pose.pose.position.x, odom.pose.pose.position.y) > 0.1 &
        ros::Time::now().toSec() - wf_reset_odom_sec > 10 & sweep_mode == 0) {
        // reset_odom = 1;
        printf("reset odom in wall follow, diff = %f, delta_Time = %f\n",
               dis_p2p(pose.pose.position.x, pose.pose.position.y,
                       odom.pose.pose.position.x, odom.pose.pose.position.y),
               ros::Time::now().toSec() - wf_reset_odom_sec);
        wf_reset_odom_sec = ros::Time::now().toSec();
    }
    ...
    if (dis_p2p(start_x, start_y, pose.pose.position.x, pose.pose.position.y) >=
            0.4 &&
        start_wall_follow == 2) {
        start_wall_follow = 3;
    }
    if (dis_p2p(start_x, start_y, pose.pose.position.x, pose.pose.position.y) <=
        ...
        if (first_bump >= 0) {
            for (int j = 0; j < CellNumber; j++) {
                if ((crosspoint[first_bump].MinBoundary_x ==
                     sweep_cell[j].MinBoundary_x) &
                    (crosspoint[first_bump].MinBoundary_y ==
                     sweep_cell[j].MinBoundary_y) &
                    (crosspoint[first_bump].MaxBoundary_x ==
                     sweep_cell[j].MaxBoundary_x) &
                    (crosspoint[first_bump].MaxBoundary_y ==
                     sweep_cell[j].MaxBoundary_y) &
                    crosspoint[first_bump].cellsep == 0) {
            ...
            if (crosspoint[first_bump].discover == 0) {
                ...
                crosspoint[first_bump].discover = 1;
                sweep_cell[CellNumber] = {minedge_x, minedge_y, maxedge_x,
                                          maxedge_y};
                CellNumber++;
            } else {
                for (int i = 0; i < 100; i++) {
                    if (crosspoint[i].discover == 0) {
                        for (int j = 0; j < CellNumber; j++) {
                            if ((crosspoint[i].MinBoundary_x ==
                                 sweep_cell[j].MinBoundary_x) &
                                (crosspoint[i].MinBoundary_y ==
                                 sweep_cell[j].MinBoundary_y) &
                                (crosspoint[i].MaxBoundary_x ==
                                 sweep_cell[j].MaxBoundary_x) &
                                (crosspoint[i].MaxBoundary_y ==
                                 sweep_cell[j].MaxBoundary_y) &
                                crosspoint[i].cellsep == 0) {
                                printf("The cell is coveraged!!\n");
                        ...
                        if (crosspoint[i].discover == 0) {
                            ...
                            crosspoint[i].discover = 1;
                            sweep_cell[CellNumber] = {minedge_x, minedge_y,
                                                      maxedge_x, maxedge_y};
                            CellNumber++;
                            break;
                        }
    ...
    if (sweep_mode == 1) {
        status = sweep_scan(pub, frequency, edgepoint_x, edgepoint_y, kp_ang,
                            ki_ang, kd_ang);
        if (status == 1) {
            // if(first_bump != 0) //?????????????????
            sweep_mode = 2;
    ...
    if ((frontdistr <= 0.32 & frontdistr >= 0.1) |
        (frontdistl <= 0.32 & frontdistl >= 0.1) |
        (frontdist1 <= 0.32 & frontdist1 >= 0.1) |
        (frontdist6 <= 0.32 & frontdist6 >= 0.1))
        stop_laser = 1;
    else if ((frontdistr <= 0.37 & frontdistr >= 0.1) |
             (frontdistl <= 0.37 & frontdistl >= 0.1))
        slow_laser = 2;
    else if ((frontdistr <= 0.42 & frontdistr >= 0.1) |
             (frontdistl <= 0.42 & frontdistl >= 0.1))
        slow_laser = 3;
    else if ((frontdistr <= 0.47 & frontdistr >= 0.1) |
             (frontdistl <= 0.47 & frontdistl >= 0.1))
        slow_laser = 4;
    else if ((frontdistr <= 0.52 & frontdistr >= 0.1) |
             (frontdistl <= 0.52 & frontdistl >= 0.1))
        slow_laser = 5;
    else
        slow_laser = 0;
    ...
    if (avoidance_status == 2 & stop_laser & start_wall_follow != 0) {
        if ((frontdistr <= 0.32 & frontdistr >= 0.1) &
            wall_stop_wf == 1) { //(frontdist1 <= 0.32 & frontdist1 >= 0.1) |
                                 //(frontdist6 <= 0.32 & frontdist6 >= 0.1)
    ...
    // printf("edgepoint_x = %f,edgepoint_y = %f,sweep_mode = %d,next_corner =
    // %d,offset_bound =
...
void findedge(float minedge_xx, float minedge_yy, float maxedge_xx,
    ...
    if ((odom.pose.pose.position.y > maxedge_yy - offset_bound) &
        last_corner != 2) {
        sweep_mode = 1;
        ...
        if (LastCrossBoundary != 4) {
            if (first_bump == -1) {
                ...
                crosspoint[sumCrossPoint] = {crosscell_x_tmp,
                                             crosscell_y_tmp,
                                             minedge_xx,
                                             maxedge_yy,
                                             maxedge_xx,
                                             maxedge_yy + celllength,
                                             0,
                                             cell_seperate};
    ...
    if ((odom.pose.pose.position.x < minedge_xx + offset_bound) &
        last_corner != 3) {
        sweep_mode = 1;
        edgepoint_x = minedge_xx;
    ...
    if ((odom.pose.pose.position.y < minedge_yy + offset_bound) &
        last_corner != 4) {
        sweep_mode = 1;
    ...
    if ((odom.pose.pose.position.x > maxedge_xx + 0.15 - offset_bound) &
        last_corner != 5) {
        sweep_mode = 1;
        edgepoint_x = maxedge_xx;
        edgepoint_y = maxedge_yy;
...
void save_cellmap() {
    ...
    if (/*image_run_onece == 1 & */ !start_wall_follow) {
        ...
        // cv::Mat erodeStruct = getStructuringElement(cv::MORPH_RECT,
        // cv::Size(erode_param, erode_param));
        // erode(graymap, graymap, erodeStruct);
        cell_status =
            cell_coordinate(graymap, (uint8_t)(1 / mapmsg.info.resolution),
                            (uint16_t)((0 - mapmsg.info.origin.position.x) /
                                       mapmsg.info.resolution),
                            (uint16_t)((0 - mapmsg.info.origin.position.y) /
                                       mapmsg.info.resolution));
        printf("Map resloution = %u, position.x = %f, position.y = %f, width = "
               "%u, height = %u\n",
               (uint8_t)(1 / mapmsg.info.resolution),
               mapmsg.info.origin.position.x, mapmsg.info.origin.position.y,
               mapmsg.info.width, mapmsg.info.height);
        // printf("imageCallback map.x = %u, map.y = %u, sin = %f, cos =
