...
int start_pose = 0,stop_sonar = 0,stop_bumper = 0, stop_laser = 0,start_laser = 0,start_bug2 = 0, slow_laser = 0,cell_status = 0, system_status = 4, odom_switch, switch_publish, reset_odom = 0, scan_match = 0,erode_param,curve_param, turn_status = 1, start_wall_follow = 1;
int robot_mode, sweep_mode = 0, next_corner = 1, last_corner, as_wall_sweep_status = 0;
float offset_bound;
float minedge_x = -2,minedge_y = -1,maxedge_x = 0.15,maxedge_y = 1,crosscell_x,crosscell_y,edgepoint_x,edgepoint_y,robot_body_x = 0.20,robot_body_y = 0.20;  //wall_follow parameters
int first_bump = 0;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub = n_slam.subscribe(odom_id, 1000, odomCallBack);///odometry/filtered
  ...
  ros::Rate loop_rate(50);
     ...
     if(robot_mode == 0)
     {
       ...
       if(start_wall_follow == 0 & !start_sweep_scan){
         start_sweep_scan = 1;
         reset_odom = 5;                
       }
       if(start_sweep_scan & reset_odom!=5)
       {
        ...
        //go dock
        if(cellpath.status == 2 & behavior.robotmode!=2)
        {
          if(status1 == 0)
...
{
	pose = msg;
  if ((reset_odom!=0 & fabs(pose.pose.position.x - odom.pose.pose.position.x) < 0.08 & fabs(pose.pose.position.y - odom.pose.pose.position.y) < 0.08) | reset_odom==5 | reset_odom==3 | reset_odom==1){
    printf("x_diff = %f, y_diff = %f\n",fabs(pose.pose.position.x - odom.pose.pose.position.x),fabs(pose.pose.position.y - odom.pose.pose.position.y));
    tf::Pose pose_tf;
    ...
    printf("Switch_odom.status = %d, yaw = %f, pose.x = %f ,pose.y = %f ,odom.x = %f ,odom.y = %f \n\n",Switch_odom.status,pose_yaw,pose.pose.position.x,pose.pose.position.y,odom.pose.pose.position.x,odom.pose.pose.position.y);
...
void cell_sweepscan(const ros::Publisher pub,const int rate, cell_path_info *sweeppath,float kp_ang,float ki_ang,float kd_ang)
{
	float x_sp = (float)(sweeppath->x-1)/stepsize, y_sp = (float)(sweeppath->y-1)/stepsize;
	int status = 1;
	static bool status_set = 0;
	if (sweeppath->tan != sweeppath->next_tan & status_set){
		status = sweep_scan(pub,rate,x_sp,y_sp,kp_ang,ki_ang,kd_ang);
		//printf("Sweep scan in  x = %d, y = %d\n",sweeppath->x,sweeppath->y);
	}	
	//else
	//status = 1;
...
int sweep_scan(ros::Publisher pub , int frequency, const float path_coordinate_x, const float path_coordinate_y,float kp_ang,float ki_ang,float kd_ang)
{
	...
	static float prior_lin = 0, prior_ang_pos = 0, prior_ang_neg = 0, ang_diff_pos = 0, ang_diff_neg = 0, ang_diff_max = 0, max_diff;
  ...
  static int acc_curve_z = 0;
    ...
    if(!(diff < diff_distance))
    {
      ...
      as_wall_sweep_status = 0;
        ...
        if (diff < max_diff/2 & reset_odom_param & scan_match){
          if ((yaw < M_PI/4 & yaw > (-1)*M_PI/4) | yaw < (-3)*M_PI/4 | yaw > 3*M_PI/4)
            reset_odom = 1;
          else
            reset_odom = 2;
          reset_odom_param = 2;
	...
	if((ang_desired_old-fabs(ang_desired) > 0.03 | ang_desired_old-fabs(ang_desired) < -0.03) & step_one == 0 & start_bug2){
		printf("step_one = %d, start bug 2\n",step_one);
    step_one = 1;
   }
	if(step_one == 1 & ang_desired_old-fabs(ang_desired) <= 0.03 & ang_desired_old-fabs(ang_desired) >= -0.03 & start_bug2){
		start_bug2 = 0;
		turn_status = 1;
		step_one = 0;
		rotate_mode = 1;
   printf("step_one = %d, end bug 2\n",step_one);
	}
  if(start_bug2){
    if(diff < 0.15){
      return 1;
      ...
      if (rotate_mode == 0)
      {
		...
		if(diff>=0.2){
		    start_bug2 = 1;
		    if (odom_switch == 0)
		  	printf("stop with obstacle in (x = %f , y = %f).......\n",odom.pose.pose.position.x,odom.pose.pose.position.y);
		    else
		    ...
		    reset_odom_param = 0;
		    if (odom_switch == 0)
		        printf("slow_laser and arrival (x = %f , y = %f).......\n",odom.pose.pose.position.x,odom.pose.pose.position.y);
		    else
		        printf("slow_laser and arrival (x = %f , y = %f).......\n",pose.pose.position.x,pose.pose.position.y);
		    return 1 ;
	    	}
  ...
  if(start_wall_follow == 1)
    start_bug2 = 1;
	if(!start_bug2 | sweep_mode == 1)
...
void wall_follow(float minedge_xx, float minedge_yy, float maxedge_xx, float maxedge_yy, ros::Publisher pub, int frequency, float kp_ang, float ki_ang, float kd_ang){
  ...
  if(start_wall_follow == 1 & (stop_laser == 1 | first_bump == 1))
  {
  ...
  if(dis_p2p(start_x,start_y,pose.pose.position.x,pose.pose.position.y) <= 0.15 && start_wall_follow == 3)
  {
    if(first_bump == 2)
    {
      maxedge_y = maxedge_yy + celllength;
      minedge_y = maxedge_yy; 
    }
    if(first_bump == 3)
    {
      minedge_x = minedge_xx - celllength;
      maxedge_x = minedge_xx;
    }
    if(first_bump == 4)
    {
      minedge_y = minedge_yy - celllength;
      maxedge_y = minedge_yy;  
    }
    if(first_bump == 5)
    {
      maxedge_x = maxedge_xx + celllength;
      minedge_x = maxedge_xx;
    }
    start_wall_follow = 0;
    stop_laser = 0;
    next_corner = 3;
	...
	for (int j = 0 ; j<=4 ; j+=1){
		//Right side
		frontdatar[j] = laser.ranges[j+355];
    sidedatar1[j] = laser.ranges[j+323];
		sidedatar2[j] = laser.ranges[j+303];
		//Left side
		frontdatal[j] = laser.ranges[j+1];
    sidedatal1[j] = laser.ranges[j+29];
		sidedatal2[j] = laser.ranges[j+53];    
	}
  //right side
	float frontdistr = FindMedian(frontdatar,5);
  float sidedistr1 = FindMedian(sidedatar1,5);
  float sidedistr2 = FindMedian(sidedatar2,5);
  //left side
	float frontdistl = FindMedian(frontdatal,5);
  ...
  if(avoidance_status == 1){
    ...
    if(smallest < 2)
    {
      ...
      if(min_index < 2){
        for (int a = 0 ; a<=6*(-min_index+2); a+=1){
				      usleep(150000);
				      sendCmdVel(pubMessage, 0, -0.8);
			        }
        } 
  ...
  if(sweep_mode == 1){
	  ...
	  if(frontdistr!=0){
	    if(frontdistr <= 0.32 | frontdistl <= 0.32)
			    stop_laser = 1;
		  else if(frontdistr <= 0.37 | frontdistl <= 0.37)
			    slow_laser = 2;
		  else if(frontdistr <= 0.42 | frontdistl <= 0.42)
			    slow_laser = 3;
		  else if(frontdistr <= 0.47 | frontdistl <= 0.47)
			    slow_laser = 4;
		  else if(frontdistr <= 0.52 | frontdistl <= 0.52)
			    slow_laser = 5;
		  else
			    slow_laser = 0;
	  }
    if(stop_laser != 1 & sweep_mode == 0)
    {
        sendCmdVel(pubMessage, 0.2, 0);
    }
	  if(avoidance_status == 2 & stop_laser == 1 & start_wall_follow != 0){
		  if(frontdistr <= 0.32 & frontdistr!=0 & wall_stop_wf == 1){
       ...
       if(status < 1){
         ...
         wall_follow_secs = ros::Time::now().toSec();
          ...
          //printf("turn 90\n");
			  ...
			  if(sidedistr2<0.2)
				  x_speed = 0.02;
			  sendCmdVel(pubMessage, 0.14, z_speed);
			  wall_stop_wf = 1;
        if(next_corner != 1){        
...
void findedge(float minedge_xx, float minedge_yy, float maxedge_xx, float maxedge_yy)
{
  if(pose.pose.position.y > maxedge_yy - offset_bound & last_corner != 2)
  {
    sweep_mode = 1;
    ...
    {
      first_bump = 2;
      crosscell_x = pose.pose.position.x - robot_body_x;
      crosscell_y = maxedge_yy + robot_body_y;
    }
  }
  if(pose.pose.position.x < minedge_xx + offset_bound & last_corner != 3)
  {
    sweep_mode = 1;
    edgepoint_x = minedge_xx;
    ...
    {
      first_bump = 3;
      crosscell_x = minedge_xx - robot_body_x;
      crosscell_y = pose.pose.position.y - robot_body_y;
    }
  }
  if(pose.pose.position.y < minedge_yy + offset_bound & last_corner != 4)
  {
    sweep_mode = 1;
    ...
    if(first_bump == 0)
    {
      first_bump = 4;
      crosscell_x = pose.pose.position.x + robot_body_x;
      crosscell_y = minedge_yy - robot_body_y;
    }
  }
  if(pose.pose.position.x > maxedge_xx - offset_bound & last_corner != 5)
  {
    sweep_mode = 1;
    edgepoint_x = maxedge_xx;
    edgepoint_y = maxedge_yy;
    ...
    {
      first_bump = 5;
      crosscell_x = maxedge_xx + robot_body_x;
      crosscell_y = pose.pose.position.y + robot_body_y;
    }    
  }
  last_corner = 1;
